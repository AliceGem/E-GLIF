...
#include "eglif_cond_alpha_multisyn_vmin.h"
...
* Recordables map
* ---------------------------------------------------------------- */
nest::RecordablesMap<eglif_cond_alpha_multisyn_vmin>
    eglif_cond_alpha_multisyn_vmin::recordablesMap_;

namespace nest {
// Override the create() method with one call to RecordablesMap::insert_()
// for each quantity to be recorded.
template <> void RecordablesMap<eglif_cond_alpha_multisyn_vmin>::create() {
  // use standard names whereever you can for consistency!
  /* generated by template org.nest.nestml.function.RecordCallback*/
  
 
  insert_("I_stc1", &eglif_cond_alpha_multisyn_vmin::get_I_stc1);

  /* generated by template org.nest.nestml.function.RecordCallback*/

  insert_("I_stc2", &eglif_cond_alpha_multisyn_vmin::get_I_stc2);


  insert_("V_th", &eglif_cond_alpha_multisyn_vmin::get_V_th);

 
  //insert_("y_0", &eglif_cond_alpha_multisyn_vmin::get_y_0);

  /* generated by template org.nest.nestml.function.RecordCallback*/

  insert_("V_m", &eglif_cond_alpha_multisyn_vmin::get_V_m);


  insert_("I_gen", &eglif_cond_alpha_multisyn_vmin::get_I_gen);

  
  insert_("sum_buffer", &eglif_cond_alpha_multisyn_vmin::get_sum_buffer);


  insert_("G1", &eglif_cond_alpha_multisyn_vmin::get_G1);


  insert_("DG1", &eglif_cond_alpha_multisyn_vmin::get_DG1);


  insert_("G2", &eglif_cond_alpha_multisyn_vmin::get_G2);


  insert_("DG2", &eglif_cond_alpha_multisyn_vmin::get_DG2);


  insert_("G3", &eglif_cond_alpha_multisyn_vmin::get_G3);


  insert_("DG3", &eglif_cond_alpha_multisyn_vmin::get_DG3);
  ...
  //insert_("time", &eglif_cond_alpha_multisyn_vmin::get_time);
...
eglif_cond_alpha_multisyn_vmin::Parameters_::Parameters_() {}

eglif_cond_alpha_multisyn_vmin::State_::State_() {}

/* ----------------------------------------------------------------
* Parameter and state extractions and manipulation functions
* ---------------------------------------------------------------- */

void eglif_cond_alpha_multisyn_vmin::Parameters_::set(
    const DictionaryDatum &__d) {}

void eglif_cond_alpha_multisyn_vmin::State_::set(const DictionaryDatum &__d,
                                                   const Parameters_ &p) {}

eglif_cond_alpha_multisyn_vmin::Buffers_::Buffers_(
    eglif_cond_alpha_multisyn_vmin &n)
    : logger_(n), s_(0), c_(0), e_(0) {}

eglif_cond_alpha_multisyn_vmin::Buffers_::Buffers_(
    const Buffers_ &, eglif_cond_alpha_multisyn_vmin &n)
    : logger_(n), s_(0), c_(0), e_(0) {}
...
eglif_cond_alpha_multisyn_vmin::eglif_cond_alpha_multisyn_vmin()
...
eglif_cond_alpha_multisyn_vmin::eglif_cond_alpha_multisyn_vmin(
    const eglif_cond_alpha_multisyn_vmin &n)
...
void eglif_cond_alpha_multisyn_vmin::init_state_(
    const Node &proto) { // TODO inner components

  const eglif_cond_alpha_multisyn_vmin &pr =
      downcast<eglif_cond_alpha_multisyn_vmin>(proto);
  S_ = pr.S_;
}

/* generated by template org.nest.nestml.function.GSLDifferentiationFunction*/
extern "C" inline int
eglif_cond_alpha_multisyn_vmin_dynamics(double, const double y[], double f[],
                                          void *pnode) {

  //const double tIe = nest::kernel().simulation_manager.get_time().get_ms();

  typedef eglif_cond_alpha_multisyn_vmin::State_ State_;
  // get access to node so we can almost work as in a member function
  assert(pnode);
  const eglif_cond_alpha_multisyn_vmin &node =
      *(reinterpret_cast<eglif_cond_alpha_multisyn_vmin *>(pnode));
...
void eglif_cond_alpha_multisyn_vmin::init_buffers_() {
  ...
  B_.sys_.function = eglif_cond_alpha_multisyn_vmin_dynamics;
  B_.sys_.jacobian = NULL;
  B_.sys_.dimension = state_size;
  B_.sys_.params = reinterpret_cast<void *>(this);
}

void eglif_cond_alpha_multisyn_vmin::calibrate() {
  B_.logger_.init();
  ...
  for (long i = 0; i < get_bT_size(); i++) {
    V_.bufferT[i] = 0;				// Non Ã¨ qui il problema
  }
 ...
 */
void eglif_cond_alpha_multisyn_vmin::update(nest::Time const &origin,
	...
	if ( ((double) rand() / (RAND_MAX))			//V_.rng_->drand()		
        ...
        {									/////////// Spike generation!!!! ///////////
	     ...
	     spikes_times_.push_back(S_.time);
	      ...
	      // Update of Vth based on the frequency of last 3 spikes: Vth = Vth_start+constant*(f-f_tonic); e.g. constant = a, b, c (già definiti nel modello)
...
void eglif_cond_alpha_multisyn_vmin::handle(nest::DataLoggingRequest &e) {
  B_.logger_.handle(e);
}

void eglif_cond_alpha_multisyn_vmin::handle(nest::SpikeEvent &e) {
...
void eglif_cond_alpha_multisyn_vmin::handle(nest::CurrentEvent &e) {
  assert(e.get_delay() > 0);
