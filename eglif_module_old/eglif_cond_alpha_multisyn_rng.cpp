...
/*
*  eglif_cond_alpha_multisyn_rng.cpp
*
...
#include "eglif_cond_alpha_multisyn_rng.h"

using namespace nest;

std::vector <float> _spikes_times_;
/* ----------------------------------------------------------------
* Recordables map
* ---------------------------------------------------------------- */
nest::RecordablesMap< eglif_cond_alpha_multisyn_rng >
    eglif_cond_alpha_multisyn_rng::recordablesMap_;
...
template <> 
void RecordablesMap< eglif_cond_alpha_multisyn_rng >::create() {
  // use standard names whereever you can for consistency!
  /* generated by template org.nest.nestml.function.RecordCallback*/
  
 
  insert_("I_stc1", &eglif_cond_alpha_multisyn_rng::get_I_stc1);

  /* generated by template org.nest.nestml.function.RecordCallback*/

  insert_("I_stc2", &eglif_cond_alpha_multisyn_rng::get_I_stc2);


  insert_("V_th", &eglif_cond_alpha_multisyn_rng::get_V_th);

 
  //insert_("y_0", &eglif_cond_alpha_multisyn_rng::get_y_0);


  insert_("V_m", &eglif_cond_alpha_multisyn_rng::get_V_m);


  insert_("I_gen", &eglif_cond_alpha_multisyn_rng::get_I_gen);

  
  insert_("sum_buffer", &eglif_cond_alpha_multisyn_rng::get_sum_buffer);


  insert_("G1", &eglif_cond_alpha_multisyn_rng::get_G1);


  insert_("DG1", &eglif_cond_alpha_multisyn_rng::get_DG1);


  insert_("G2", &eglif_cond_alpha_multisyn_rng::get_G2);


  insert_("DG2", &eglif_cond_alpha_multisyn_rng::get_DG2);


  insert_("G3", &eglif_cond_alpha_multisyn_rng::get_G3);


  insert_("DG3", &eglif_cond_alpha_multisyn_rng::get_DG3);
  ...
  //insert_("time", &eglif_cond_alpha_multisyn_rng::get_time);
...
eglif_cond_alpha_multisyn_rng::Parameters_::Parameters_() {}

eglif_cond_alpha_multisyn_rng::State_::State_() {}

/* ----------------------------------------------------------------
* Parameter and state extractions and manipulation functions
* ---------------------------------------------------------------- */

void eglif_cond_alpha_multisyn_rng::Parameters_::set(
    const DictionaryDatum &__d) {}

void eglif_cond_alpha_multisyn_rng::State_::set(const DictionaryDatum &__d,
                                                   const Parameters_ &p) {}

eglif_cond_alpha_multisyn_rng::Buffers_::Buffers_(
    eglif_cond_alpha_multisyn_rng &n)
    : logger_(n), s_(0), c_(0), e_(0) {}

eglif_cond_alpha_multisyn_rng::Buffers_::Buffers_(
    const Buffers_ &, eglif_cond_alpha_multisyn_rng &n)
    : logger_(n), s_(0), c_(0), e_(0) {}
...
eglif_cond_alpha_multisyn_rng::eglif_cond_alpha_multisyn_rng()
...
eglif_cond_alpha_multisyn_rng::eglif_cond_alpha_multisyn_rng(
    const eglif_cond_alpha_multisyn_rng &n)
...
void eglif_cond_alpha_multisyn_rng::init_state_(
    const Node &proto) { // TODO inner components

  const eglif_cond_alpha_multisyn_rng &pr =
      downcast<eglif_cond_alpha_multisyn_rng>(proto);
  S_ = pr.S_;
}

/* generated by template org.nest.nestml.function.GSLDifferentiationFunction*/
extern "C" inline int
eglif_cond_alpha_multisyn_rng_dynamics(double, const double y[], double f[],
                                          void *pnode) {

  //const double tIe = nest::kernel().simulation_manager.get_time().get_ms();

  typedef eglif_cond_alpha_multisyn_rng::State_ State_;
  // get access to node so we can almost work as in a member function
  assert(pnode);
  const eglif_cond_alpha_multisyn_rng &node =
      *(reinterpret_cast<eglif_cond_alpha_multisyn_rng *>(pnode));
...
void 
eglif_cond_alpha_multisyn_rng::init_buffers_() {
  ...
  B_.sys_.function = eglif_cond_alpha_multisyn_rng_dynamics;
...
void 
eglif_cond_alpha_multisyn_rng::calibrate() {
  B_.logger_.init();
 ...
 */
void 
eglif_cond_alpha_multisyn_rng::update(nest::Time const &origin,
   ...
   //  }
	...
	if (V_.rng_->drand()		
        ...
        {									/////////// Spike generation!!!! ///////////
	     ...
	     _spikes_times_.push_back(S_.time);
		...
		int num_spk = 0;
		for (int i = 1; i<_spikes_times_.size(); i++){
			if (_spikes_times_[i-1]>lim_inf && _spikes_times_[i-1]<lim_sup)
				num_spk += 1;
		}
...
void eglif_cond_alpha_multisyn_rng::handle(nest::DataLoggingRequest &e) {
  B_.logger_.handle(e);
}

void 
eglif_cond_alpha_multisyn_rng::handle(nest::SpikeEvent &e) {
...
void 
eglif_cond_alpha_multisyn_rng::handle(nest::CurrentEvent &e) {
  assert(e.get_delay() > 0);
