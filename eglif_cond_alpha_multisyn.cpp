...
template <> 
void RecordablesMap< eglif_cond_alpha_multisyn >::create() {
...
void 
eglif_cond_alpha_multisyn::init_buffers_() {
...
void 
eglif_cond_alpha_multisyn::calibrate() {
  B_.logger_.init();
 ...
 */
void 
eglif_cond_alpha_multisyn::update(nest::Time const &origin,
   ...
   //  }
	...
	if ( ((double) rand() / (RAND_MAX))			//V_.rng_->drand()		
        ...
        {									/////////// Spike generation!!!! ///////////
	     ...
	     spikes_times.push_back(S_.time);
		...
		int num_spk = 0;
		for (int i = 1; i<spikes_times.size(); i++){
			if (spikes_times[i-1]>lim_inf && spikes_times[i-1]<lim_sup)
				num_spk += 1;
		}
...
void 
eglif_cond_alpha_multisyn::handle(nest::SpikeEvent &e) {
...
void 
eglif_cond_alpha_multisyn::handle(nest::CurrentEvent &e) {
  assert(e.get_delay() > 0);
